...
def read_xlsx_cas_all(excel_path):
    sheet = pd.read_excel(excel_path)
    name_rule_2 = pd.read_excel(excel_path,
                                sheet_name="naming rule2")
    name_rule_1 = pd.read_excel(excel_path,
    ...
    for i in range(len(label_interval)):
        if label_interval[i] != ",":
            tmp2.append(expresses[i])
            tmp.append(label_interval[i])
        else:
            label.append(tmp)
            label_express.append(tmp2)
            tmp, tmp2 = [], []
    path = []
    tmp = "###"
    for i in path_list:
        if i != tmp:
            path.append(i)
            tmp = i
    return label, label_express, path
...
def load_label_all(args):
    def recursive_merge(inter, express, start_index=0):
        for i in range(start_index, len(inter) - 1):
            if inter[i][1] > inter[i + 1][0]:
                new_start = inter[i][0]
                new_end = max(inter[i][1], inter[i + 1][1])
                inter[i] = [new_start, new_end]
                if express[i] == 'other': express[i] = express[i + 1]
                del inter[i + 1]
                del express[i + 1]
                return recursive_merge(inter.copy(), express.copy(), start_index=i)
        return inter, express

    label_inter, label_express, label_path = read_xlsx_cas_all(args.path_xlsx)
    merge_inters, merge_expresses = [], []
    for i in range(len(label_inter)):
        inter, express = recursive_merge(label_inter[i], label_express[i])
        merge_inters.append(inter)
        merge_expresses.append(express)
    return merge_inters, merge_expresses, label_path
