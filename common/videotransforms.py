...
class ImgEnhance(object):
    ...
    def aug(self, image, value):
        if random.random() < 0.25:
            enh_img = ImageEnhance.Brightness(image)
        elif random.random() < 0.5:
            enh_img = ImageEnhance.Color(image)
        elif random.random() < 0.75:
            enh_img = ImageEnhance.Contrast(image)
        else:
            enh_img = ImageEnhance.Sharpness(image)
        image_ = enh_img.enhance(value)
