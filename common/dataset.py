...
def get_class_index_map(class_info_path='../datasets/ClassIndex.txt'):
...
def load_video_data(video_infos, npy_data_path):
    ...
    for video_name in tqdm.tqdm(list(video_infos.keys()), ncols=0):
        npy_path = '%s/%s.npy' % (npy_data_path, video_name)
        data = np.load(npy_path)
        data_dict[video_name] = data
        # data_dict[video_name] = npy_path
    return data_dict
...
class THUMOS_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # npy_path = self.data_dict[sample_info['video_name']]
        ...
        # if self.training:
        #     input_data = self.random_enhance(input_data)
